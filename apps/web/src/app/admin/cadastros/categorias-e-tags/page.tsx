...
export default function AdminCadastrosCategoriasETagsPage() {
    ...
    try {
      ...
      if (!res.ok) {
        const details = (await res.json().catch(() => null)) as
          ...
          | null;
        throw new Error(
          details?.error ?? "Não foi possível carregar as categorias.",
        );
      }
  ...
  async function handleUpdateGroup(e: React.FormEvent) {
    ...
    try {
      const res = await fetch(
        `/api/admin/category-groups?id=${encodeURIComponent(editingGroupId)}`,
        {
          method: "PUT",
          headers: { "content-type": "application/json" },
    ...
    try {
      ...
      if (!res.ok) {
        const details = (await res.json().catch(() => null)) as
          ...
          | null;
        throw new Error(
          details?.error ?? "Não foi possível cadastrar a categoria.",
        );
      }
    ...
    try {
      ...
      if (!res.ok) {
        const details = (await res.json().catch(() => null)) as
          ...
          | null;
        throw new Error(
          details?.error ?? "Não foi possível atualizar a categoria.",
        );
      }
  ...
  async function handleDeleteCategory(categoryId: string) {
    ...
    try {
      const res = await fetch(
        `/api/admin/categories?id=${encodeURIComponent(categoryId)}`,
        {
          method: "DELETE",
        },
      );
      ...
      if (!res.ok) {
        const details = (await res.json().catch(() => null)) as
          ...
          | null;
        throw new Error(
          details?.error ?? "Não foi possível excluir a categoria.",
        );
      }
        ...
        <div className="rounded-lg bg-white p-6 shadow-sm ring-1 ring-slate-200">
              ...
              <div className="max-h-[520px] overflow-auto p-2">
                {groupsLoading || categoriesLoading ? (
                  ...
                  <ul className="space-y-1">
                    {groups.map((g) => {
                      const catsCount = categories.filter(
                        (c) => c.group_id === g.id,
                      ).length;
                      const isActive = g.id === activeGroupId;
                      const isFixed = g.name.trim() === "Estilo Musical";
          ...
          <Dialog
            ...
            onOpenChange={(open) => {
              ...
              const isFixed = group?.name.trim() === "Estilo Musical";
              if (isFixed) return;

              // Mantém o comportamento atual quando abrir via menu
            }}
          >
            <DialogContent className="sm:max-w-lg">
              <DialogHeader>
              ...
              <form onSubmit={handleUpdateGroup} className="grid gap-4">
                <div className="grid gap-2">
                  ...
                  <Input
                    ...
                    placeholder="Ex: Eventos"
                    disabled={editingGroupName.trim() === "Estilo Musical"}
                  />
                </div>
          ...
          <Dialog
            ...
            onOpenChange={(open) => (!open ? setEditingCategoryId(null) : null)}
          >
            <DialogContent className="sm:max-w-lg">
              <DialogHeader>
